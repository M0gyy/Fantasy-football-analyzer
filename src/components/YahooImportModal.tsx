...
export const YahooImportModal: React.FC<YahooImportModalProps> = ({
  ...
  React.useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'OAUTH_AUTH_SUCCESS' && event.data?.provider === 'yahoo') {
        handleImport('Yahoo Champions League', 'PPR');
      }
    };
    window.addEventListener('message', handleMessage);
    return () => window.removeEventListener('message', handleMessage);
  }, [userTeamName, teamNumber, leagueId]);

  const handleYahooOAuthConnect = async () => {
    // Synchronously create popup reference on user click gesture
    ...
    try {
      setIsLoading(true);
      setStep('AUTH');
      setProgressMsg('Connecting to Yahoo! OAuth 2.0 gateway...');

      const res = await fetch('/api/auth/yahoo/url');
      if (!res.ok) throw new Error('Failed to retrieve Yahoo auth URL');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn font-mono">
      <div className="bg-zinc-950 border border-purple-900/60 rounded-xl w-full max-w-lg p-5 space-y-4 shadow-2xl shadow-purple-950/40 relative overflow-hidden">
        ...
        {/* Yahoo! Direct OAuth 2.0 Connect Action */}
        {!isLoading && step === 'IDLE' && (
          <div className="p-3.5 bg-gradient-to-r from-purple-950/80 via-purple-900/50 to-indigo-950/80 border border-purple-700/60 rounded-xl space-y-2.5 shadow-md">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-purple-500 text-white font-bold text-xs flex items-center justify-center">
                  Y!
                </span>
                <span className="text-xs font-bold text-purple-200">
                  Direct Yahoo! OAuth 2.0 Authentication
                </span>
              </div>
              <span className="text-[10px] bg-purple-900/80 text-purple-300 border border-purple-600/50 px-2 py-0.5 rounded font-mono">
                OAuth 2.0
              </span>
            </div>
            <p className="text-[11px] text-zinc-300 font-sans leading-relaxed">
              Authenticate directly with your Yahoo account to import your fantasy league, custom scoring rules, and current week starting lineup.
            </p>
            <button
              onClick={handleYahooOAuthConnect}
              type="button"
              className="w-full py-2 px-4 bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs rounded-lg transition-all shadow-lg shadow-purple-600/40 flex items-center justify-center gap-2 cursor-pointer border border-purple-400/30"
            >
              <Zap className="w-4 h-4 text-amber-300 animate-pulse" />
              <span>Log in & Connect Yahoo! Account</span>
            </button>
          </div>
        )}
          ...
          <div className="py-8 px-4 bg-purple-950/20 border border-purple-800/40 rounded-lg flex flex-col items-center justify-center text-center space-y-3">
            ...
            <div className="space-y-1 max-w-sm">
              ...
              <p className="text-[10px] text-zinc-400 font-mono">
                {step === 'AUTH' && 'Authenticating OAuth 2.0 token...'}
            ...
            {step === 'AUTH' && authUrl && (
              <div className="pt-2 space-y-2">
                <a
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-800 hover:bg-purple-700 text-purple-100 text-xs font-semibold rounded-md border border-purple-600 transition-colors shadow-sm"
                >
                  <span>Open Yahoo! Login Page in New Tab</span>
                  <ExternalLink className="w-3.5 h-3.5" />
                </a>
                <div>
                  <button
                    type="button"
                    onClick={() => handleImport('Yahoo Champions League', 'PPR')}
                    className="text-[11px] text-purple-400 hover:text-purple-300 underline font-mono cursor-pointer"
                  >
                    Or skip to import with current credentials →
                  </button>
                </div>
              </div>
            )}
